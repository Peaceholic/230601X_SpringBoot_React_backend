package com.hello.proto.filter;
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
		...
		try {
			...
			if (token != null && !token.equalsIgnoreCase("null")) {
				...
				// Token payload에서 userEmail 가져오기
				String userEmail = tokenProvider.validateToken(token);

				// SecurityContext에 객체 추가
				...
				// SecurityContext에 AbstractAuthenticationToken 추가 해당 Thread가 지속적인 인증을 가질수 있도록 설정
